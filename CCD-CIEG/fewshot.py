...
import torch
from openprompt.data_utils.text_classification_dataset import PengProcessor
...
parser.add_argument("--model", type=str, default='bert')#roberta
parser.add_argument("--model_name_or_path", default='F://models//bert_chinese') #bert_base_cased #chinese-roberta-wwm-ext   #bert_chinese
# parser.add_argument("--model_name_or_path", default='/home/zy-4090-1/models/roberta-base_cased')
...
#     batch_s = args.batch_size
    ...
    max_seq_l = 128
    batch_s = args.batch_size
elif args.dataset == "DLC":
    dataset['train'] = DLCProcessor().get_train_examples("datasets/TextClassification/DLC/")
    dataset['test'] = DLCProcessor().get_test_examples("datasets/TextClassification/DLC/")
    class_labels =DLCProcessor().get_labels()
    scriptsbase = "TextClassification/DLC"
    scriptformat = "txt"
    cutoff=0.5
    max_seq_l = 128
    batch_s = args.batch_size
elif args.dataset == "SCC":
    dataset['train'] = SCCProcessor().get_train_examples("datasets/TextClassification/SCC/")
    dataset['test'] = SCCProcessor().get_test_examples("datasets/TextClassification/SCC/")
    class_labels =SCCProcessor().get_labels()
    scriptsbase = "TextClassification/SCC"
    scriptformat = "txt"
    cutoff=0.5
    max_seq_l = 128
    batch_s = args.batch_size
elif args.dataset == "W23C":
    dataset['train'] = W23CProcessor().get_train_examples("datasets/TextClassification/W23C/")
    dataset['test'] = W23CProcessor().get_test_examples("datasets/TextClassification/W23C/")
    class_labels =W23CProcessor().get_labels()
    scriptsbase = "TextClassification/W23C"
    scriptformat = "txt"
    cutoff=0.5
    max_seq_l = 128
    batch_s = args.batch_size
elif args.dataset == "wechat":
    dataset['train'] = wechatProcessor().get_train_examples("datasets/TextClassification/wechat/")
    dataset['test'] = wechatProcessor().get_test_examples("datasets/TextClassification/wechat/")
    class_labels =wechatProcessor().get_labels()
    scriptsbase = "TextClassification/wechat"
    scriptformat = "txt"
    cutoff=0.5
    max_seq_l = 128
    batch_s = args.batch_size
elif args.dataset == "tencent":
    dataset['train'] = TenProcessor().get_train_examples("datasets/TextClassification/tencent/")
    dataset['test'] = TenProcessor().get_test_examples("datasets/TextClassification/tencent/")
    class_labels =TenProcessor().get_labels()
    scriptsbase = "TextClassification/tencent"
    scriptformat = "txt"
    cutoff=0.5
    max_seq_l = 128
    batch_s = args.batch_size
elif args.dataset == "tencent2":
    dataset['train'] = TenProcessor2().get_train_examples("datasets/TextClassification/tencent2/")
    dataset['test'] = TenProcessor2().get_test_examples("datasets/TextClassification/tencent2/")
    class_labels =TenProcessor2().get_labels()
    scriptsbase = "TextClassification/tencent2"
    scriptformat = "txt"
    cutoff=0.5
    max_seq_l = 128
    batch_s = args.batch_size
elif args.dataset == "pengpai":
    dataset['train'] = PengProcessor().get_train_examples("datasets/TextClassification/pengpai/")
    dataset['test'] = PengProcessor().get_test_examples("datasets/TextClassification/pengpai/")
    class_labels =PengProcessor().get_labels()
    scriptsbase = "TextClassification/pengpai"
    scriptformat = "txt"
    cutoff=0.5
    max_seq_l = 128
    batch_s = args.batch_size
elif args.dataset == "zongxiang":
    dataset['train'] = ZongProcessor().get_train_examples("datasets/TextClassification/zongxiang/")
    dataset['test'] = ZongProcessor().get_test_examples("datasets/TextClassification/zongxiang/")
    class_labels =ZongProcessor().get_labels()
    ...
    dataset['train'] = WangProcessor().get_train_examples("datasets/TextClassification/wangyi/")
    dataset['test'] = WangProcessor().get_test_examples("datasets/TextClassification/wangyi/")
    class_labels =WangProcessor().get_labels()
...
elif args.verbalizer == "ivpl":
    myverbalizer = KnowledgeableVerbalizer(tokenizer, classes=class_labels,candidate_frac=cutoff,
                                           pred_temp=args.pred_temp, max_token_split=args.max_token_split).from_file(
        path=f"./scripts/{scriptsbase}/ivpl_verbalizer.{scriptformat}")

# (contextual) calibration
if args.verbalizer in ["kpt", "manual",'inter','ex_re','ex_re1','sc','reverse','kpt++','ivpl']:
...
if args.verbalizer == "kpt":
    ...
    optimizer_grouped_parameters1 = [
        {'params': [p for n, p in prompt_model.plm.named_parameters() if not any(nd in n for nd in no_decay)],
         'weight_decay': 0.01},
        {'params': [p for n, p in prompt_model.plm.named_parameFters() if any(nd in n for nd in no_decay)],
         'weight_decay': 0.0}
    ]
